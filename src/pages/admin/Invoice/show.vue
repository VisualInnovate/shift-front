<template>
  <div class="min-h-screen bg-slate-50 p-4 md:p-8 font-sans text-slate-900" :dir="lang === 'ar' ? 'rtl' : 'ltr'">
    ...
    <div v-else-if="invoice" class="max-w-7xl mx-auto space-y-6">
      <!-- Top Header Bar -->
      <div
        class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border-t-4 border-[#0b3baa]"
      >
        ...
        <div class="flex gap-3">
          ...
          <Button :label="t('print')" icon="pi pi-print" @click="printInvoice" />
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
          ...
          <section class="bg-[#0b3baa] text-white p-6 rounded-2xl shadow-xl relative overflow-hidden">
            ...
            <h3 class="text-lg font-bold mb-4 text-[#F3B913] uppercase tracking-widest text-sm">
              {{ t('invoice.summary') }}
            </h3>
            <div class="space-y-3 relative z-10">
              ...
              <div class="flex justify-between opacity-80">
                ...
                <span class="text-white">{{
                  formatCurrency(parseFloat(invoice.tax_fee) + parseFloat(invoice.service_fee))
                }}</span>
              </div>
              <div class="flex justify-between opacity-80">
                <span>{{ t('order.delivery') }}</span>
                <span class="text-white">{{
                  formatCurrency(parseFloat(invoice.delivery_fee) + parseFloat(invoice.service_fee))
                }}</span>
              </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-6 border-b border-slate-100 bg-slate-50/50">
              <h3 class="text-lg font-bold text-slate-800">
                {{ t('invoice.items') }}
                <span class="text-slate-400 font-normal ml-2">({{ invoice.items?.length }})</span>
              </h3>
            </div>
            ...
            <DataTable :value="invoice.items" responsiveLayout="scroll" class="p-datatable-sm fancy-table">
                ...
                <template #body="slotProps">
                  <div class="flex flex-column py-1">
                    ...
                    <div class="flex items-center gap-2">
                      <Tag
                        :value="slotProps.data.product?.code"
                        severity="secondary"
                        class="!text-[10px] !bg-slate-100 !text-slate-500"
                      />
              ...
              <Column field="quantity" :header="t('quantity')" class="text-center">
                <template #body="slotProps">
                  <span class="bg-slate-100 px-3 py-1 rounded-lg font-black text-[#0b3baa] text-sm"
                    >x{{ slotProps.data.quantity }}</span
                  >
                </template>
              </Column>
    ...
    <div v-else class="text-center py-20 bg-white rounded-3xl shadow-sm max-w-xl mx-auto mt-10 border border-slate-100">
      <div
        class="w-20 h-20 bg-slate-100 text-slate-400 rounded-full flex items-center justify-center mx-auto mb-6 text-3xl"
      >
      ...
      <Button
        ...
        class="p-button-text !text-[#0b3baa] mt-4"
        @click="goBack"
      />
    </div>
  </div>
    ...
    <div class="print-container" :dir="lang === 'ar' ? 'rtl' : 'ltr'">
      <div class="header-grid">
        <div class="info-col">
          ...
          <p v-if="invoice?.address">
            <strong>{{ t('invoice.deliveryAddress') }}:</strong> {{ formatAddress(invoice.address) }}
          </p>
        </div>
        <div class="qr-col">
      ...
      <table class="items-table">
        ...
        <tbody>
          <tr v-for="(item, i) in invoice?.items" :key="i">
            ...
            <td class="text-right">
              <strong>{{ formatCurrency(parseFloat(item.price) * item.quantity) }}</strong>
            </td>
          </tr>
        </tbody>
      </table>
      <div class="summary-section">
        <div class="summary-box">
          <div class="summary-row">
            <span>{{ t('invoice.subTotal') }}</span> <span>{{ formatCurrency(invoice?.sub_total_price) }}</span>
          </div>
          <div class="summary-row" v-if="parseFloat(invoice?.tax_fee) > 0">
            <span>{{ t('invoice.tax') }}</span> <span>{{ formatCurrency(invoice?.tax_fee) }}</span>
          </div>
          <div class="summary-row">
            <span>{{ t('order.delivery') }}</span> <span>{{ formatCurrency(invoice?.delivery_fee) }}</span>
          </div>
...
<script setup>
  ...
  const googleMapsLink = computed(
    () => `https://www.google.com/maps?q=${invoice.value.address.lat},${invoice.value.address.long}`,
  )
