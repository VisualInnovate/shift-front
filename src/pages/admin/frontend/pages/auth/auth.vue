    ...
    <div class="bg-white rounded-3xl shadow-2xl p-4 w-full max-w-sm sm:max-w-lg my-8">
      ...
      <!-- <div class="bg-gray-100 rounded-xl p-1 mb-8 flex transition-all duration-300">
        <button
          ...
          :aria-label="t('loginPage.phoneTab')"
        >
          {{ t('loginPage.phoneTab') }}
        </button>
        <button
          ...
          :aria-label="t('loginPage.emailTab')"
        >
      ...
      <form @submit.prevent="handleLogin" v-if="selectedTab === 'phone'" class="space-y-6">
        <div class="flex">
          <select
            ...
            required
          >
            <option value="" disabled>{{ t('loginPage.selectCountry') }}</option>
        ...
        >
          {{ authStore.loading ? t('loginPage.loading') : t('loginPage.submit') }}
        </button>
      ...
      <div class="flex items-center my-6">
        ...
        <span class="mx-4 text-gray-400 text-xs font-semibold">{{ t('loginPage.or') }}</span>
        <div class="flex-grow border-t border-gray-300"></div>
      </div>
