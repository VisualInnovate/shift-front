...
<script setup>
  ...
  // Disabled DOM meta helpers so `useHead` result can be tested without interference.
  const getMetaTag = () => null
  const createMetaTag = () => null
  const setMetaTag = () => {}
  ...
  const updateProductMeta = () => {
    // no-op while testing `useHead` elsewhere.
  }
