  ...
  <div class="product-page max-w-7xl mx-auto p-4 lg:p-8">
    ...
    <header v-else class="flex flex-col lg:flex-row items-start gap-8 lg:gap-12">
      ...
      <div class="w-full lg:w-1/2 flex flex-col-reverse lg:flex-row items-center gap-4">
        <div
          class="flex lg:flex-col flex-row items-center justify-center lg:justify-start gap-3 w-full lg:w-1/4 order-2 lg:order-1"
        >
          <img
            ...
            :class="{
              'border-blue-500 opacity-100': currentImg === img.url,
              'border-gray-300 opacity-70': currentImg !== img.url,
            }"
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 lg:p-8">
          ...
          <div class="mb-8">
            <div class="flex items-baseline gap-4 flex-wrap">
              ...
              <span class="text-4xl font-bold text-gray-900"> {{ finalPrice }} {{ $t('currencyLabel') }} </span>
        ...
        <div v-for="review in displayedReviews" :key="review.id" class="border-b pb-8 last:border-0">
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-4">
              <div
                class="w-12 h-12 bg-gray-200 rounded-full flex items-center justify-center text-xl font-bold text-gray-600"
              >
...
<script setup>
  ...
  const getMetaTag = (attrName, attrValue) => document.head.querySelector(`meta[${attrName}="${attrValue}"]`)
  ...
  const setMetaTag = (attrName, attrValue, content) => {
    const existing = getMetaTag(attrName, attrValue)
    if (existing) {
      existing.setAttribute('content', content)
      return
    }
    createMetaTag(attrName, attrValue, content)
  }
  ...
  const getShareTitle = () => {
    return locale.value === 'en'
      ? pro.value.name_en || pro.value.name_ar || 'Shift7'
      : pro.value.name_ar || pro.value.name_en || 'Shift7'
  }

  const getShareDescription = () => {
    const description = locale.value === 'en' ? pro.value.description_en : pro.value.description_ar
    if (description) return description
    return locale.value === 'en'
      ? pro.value.sub_name_en || pro.value.name_en || ''
      : pro.value.sub_name_ar || pro.value.name_ar || ''
  }

  const getShareImage = () => {
    return currentImg.value || pro.value.key_default_image || allImages.value[0]?.url || defaultPlaceholder
  }
  ...
  const hasPartialSelection = computed(
    () => Object.keys(selectedAttributes.value).length > 0 && selectedVariant.value === null,
  )

  const fullDescription = computed(() =>
    locale.value === 'en'
      ? pro.value.description_en || t('product.noDescription')
      : pro.value.description_ar || t('product.noDescription'),
  )
  ...
  const formatDate = (date) =>
    new Date(date).toLocaleDateString(locale.value === 'ar' ? 'ar-EG' : 'en-US', {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
    })
  ...
  const selectStore = (store) =>
    store && router.push(store.has_market ? { name: 'stores-hasmarket' } : { name: 'home' })
  ...
  const fetchProductData = async () => {
    ...
    try {
      ...
      if (data.is_success) {
        ...
        currentImg.value =
          currentImg.value || pro.value.key_default_image || allImages.value[0]?.url || defaultPlaceholder
        quantity.value = pro.value.in_cart ? pro.value.quantity || 1 : 1
        updateProductMeta()
      } else {
  ...
  watch([currentImg, locale], () => {
    if (!isLoading.value && !error.value && pro.value?.id) {
      updateProductMeta()
    }
  })
