<script setup>
  ...
  const isMarketVisible = (value) => isEnabledValue(value)
  const isMarketBusy = (value) => isEnabledValue(value)
  const toVisibleValue = (value) => (isMarketVisible(value) ? 1 : 0)
  const toBusyValue = (value) => (isMarketBusy(value) ? 1 : 0)
  const getToggleResponseValue = (response, key) => {
    ...
    if (['boolean', 'number', 'string'].includes(typeof data)) return data
    if (['boolean', 'number', 'string'].includes(typeof payload)) return payload
  ...
  const deleteMarket = () => {
    axios
      .delete(`/api/market/${deleteId.value}`)
      .then(() => {
        toast.add({
          severity: 'success',
          summary: t('success'),
  ...
  const downloadExample = () => {
    const csvContent =
      'id,Arabic Name,English Name,Top Parent / Store\n' +
      exampleData.value
        .map((row) => `"${row.id}","${row.arabic_name}","${row.english_name}","${row.Top_Parent}",`)
        .join('\n')
  ...
  const importMarkets = () => {
    ...
    axios
      .post('/api/import/markets', formData)
      .then(() => {
        toast.add({
          severity: 'success',
          summary: t('success'),
  ...
  const addservice_fee = () => {
    axios
      .post(`/api/market/service/fee/${selectedMarketId.value}`, {
  ...
  const toggleCloseMarket = async (market, value) => {
    ...
    try {
      ...
      const updatedValue = getToggleResponseValue(response, 'is_busy')
  ...
  async function toggleShowMarket(market, value) {
    const marketId = market.id
    const previousValue = isMarketVisible(market.is_visible)
    const nextValue = Boolean(value)

    if (showMarketLoading.value[marketId]) return

    showMarketLoading.value[marketId] = true
    market.is_visible = nextValue ? 1 : 0

    try {
      const response = await axios.post(`/api/market/is_visible/${marketId}`)
      const updatedValue = getToggleResponseValue(response, 'is_visible')

      if (updatedValue !== undefined && updatedValue !== null) {
        market.is_visible = toVisibleValue(updatedValue)
      }
    } catch (error) {
      market.is_visible = previousValue ? 1 : 0
      toast.add({ severity: 'error', summary: t('error'), detail: t('market.showStoreError'), life: 3000 })
    } finally {
      showMarketLoading.value[marketId] = false
    }
  }
      ...
      <div class="card p-4 shadow-2 border-round">
        ...
        <div class="card shadow-1 surface-0">
          ...
          >
            ...
            <Column
              field="store.service_fee"
              :header="t('market.serviceFee')"
              :sortable="true"
              header-style="width:14%; min-width:10rem;"
            >
              <template #body="slotProps"> {{ slotProps.data?.service_fee || 0 }} {{ $t('currencyLabel') }} </template>
            </Column>
            <Column
              field="is_busy"
              :header="t('market.closeStore')"
              :sortable="true"
            ...
            >
              <template #body="slotProps">
                <div class="flex justify-content-center align-items-center">
                  <InputSwitch
                    :model-value="isMarketVisible(slotProps.data.is_visible)"
                    :disabled="showMarketLoading[slotProps.data.id]"
                    @update:model-value="(value) => toggleShowMarket(slotProps.data, value)"
                  />
                  <ProgressSpinner
                    v-if="showMarketLoading[slotProps.data.id]"
                    style="width: 20px; height: 20px"
                    class="ml-2"
                    strokeWidth="5"
                  />
              ...
              <template #body="slotProps">
                <div class="grid grid-cols-2 gap-1 w-fit mx-auto">
                  <Button
                    ...
                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-emerald-500 hover:bg-emerald-50 hover:text-emerald-600 dark:hover:bg-emerald-950/40 transition-all duration-150 hover:scale-110 cursor-pointer"
                  />
                  ...
                  <Button
                    ...
                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-red-500 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/40 transition-all duration-150 hover:scale-110 cursor-pointer"
                  />
                  ...
                  <Button
                    ...
                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-amber-500 hover:bg-amber-50 hover:text-amber-600 dark:hover:bg-amber-950/40 transition-all duration-150 hover:scale-110 cursor-pointer"
                  />
                  ...
                  <Button
                    ...
                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-blue-500 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-950/40 transition-all duration-150 hover:scale-110 cursor-pointer"
                  />
                  ...
                  <Button
                    ...
                    v-tooltip.top="t('market.timeslots')"
                    aria-label="Market time slots"
                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-cyan-500 hover:bg-cyan-50 hover:text-cyan-600 dark:hover:bg-cyan-950/40 transition-all duration-150 hover:scale-110 cursor-pointer"
                  />
                </div>
              </template>
            </Column>
        ...
        >
          <div class="gap-3">
            ...
            <InputNumber
              class="w-full"
              id="service_final_price"
              v-model="service_final_price"
              :min="0"
              :step="0.01"
              mode="decimal"
              :minFractionDigits="0"
        ...
        <Dialog
          ...
          :header="`${t('market.timeslots')} - ${getMarketDisplayName(selectedTimeslotMarket)}`"
          :modal="true"
        >
            ...
            <div class="timeslot-form grid align-items-end gap-4">
              <div class="col-12 md:col-4">
                ...
                <Dropdown
                  ...
                  optionLabel="label"
                  optionValue="value"
                  :placeholder="t('market.selectDay')"
                  class="w-full"
