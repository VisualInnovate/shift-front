...
const fetchMarketById = async (id) => {
  try {
    ...
    market.media.forEach(media => {
      if (media.name === 'market_image') {
...
const submitForm = async () => {
  ...
  try {
    const marketId = route.params.id; // Get ID from route params
    const method = marketId ? 'post' : 'post'; // Use PUT for update, POST for create
    const url = marketId ? `/api/market/${marketId}` : '/api/market';
    ...
    toast.add({
      severity: 'success',
      summary: t("success"),
      detail: marketId ? t('market.updateSuccess') : t('market.createSuccess'),
      life: 3000
    });
  } catch (error) {
    ...
    toast.add({
      severity: 'error',
      summary: t("error"),
      detail: error.response?.data?.message || (route.params.id ? t('market.updateError') : t('market.createError')),
      life: 3000
    });
  } finally {
    loading.value = false;
...
onMounted(async () => {
  ...
  if (marketId) {
    await fetchMarketById(marketId); // Fetch data for update
  }
});
</script>
...
<template>
  <div v-can="'create markets'" class="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-lg">
    <h1 class="text-3xl font-bold text-center mb-8 text-gray-800">
      {{ route.params.id ? t('market.updateTitle') : t('market.createTitle') }}
    </h1>
    <form @submit.prevent="submitForm" class="space-y-6">
      ...
      <div class="pt-4 flex justify-center space-x-4">
        ...
        <Button
          type="submit"
          :label="route.params.id ? t('market.updateButton') : t('market.createButton')"
          icon="pi pi-plus"
          :loading="loading"
          ...
          :disabled="loading"
        >
          <span v-if="!loading">{{ route.params.id ? t('market.updateButton') : t('market.createButton') }}</span>
