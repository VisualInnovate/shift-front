...
const fetchShippingSetting = async () => {
  loading.value = true;
  try {
    const response = await axios.get(`/api/product/shipping/setting/show/${route.params.id}`);
    const data = response.data.data.product_shipping_setting;
    shippingData.value.product_id=route.params.id
    if(response.data.data.product_shipping_setting)
    shippingData.value = {
...
const submitForm = async () => {
  ...
  try {
    ...
    await axios.post(`/api/product/shipping/setting/update`, shippingData.value, {
