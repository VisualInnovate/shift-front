<template>
  <div class="min-h-screen bg-gray-50 p-6 lg:p-10" :dir="$t('dir')">
    ...
    <div class="card-fancy mb-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div class="field-group">
          ...
          <InputText v-model="newDetail.name_en" :placeholder="$t('custom_tabs.enter_name_en')" class="w-full"
        ...
        <div class="field-group">
          ...
          <InputText v-model="newDetail.name_ar" :placeholder="$t('custom_tabs.enter_name_ar')" class="w-full"
        ...
        <div class="field-group">
          <label class="input-label"> {{ $t('custom_tabs.type') }} <span class="text-red-500">*</span> </label>
          <Dropdown v-model="newDetail.type" :options="detailTypeOptions" optionLabel="label" optionValue="value"
            :placeholder="$t('custom_tabs.select_type')" class="w-full"
        ...
        <div v-if="newDetail.type === 1" class="field-group">
          ...
          <Dropdown v-model="newDetail.model_id" :options="modelOptions" :optionLabel="modelOptionLabel"
            optionValue="id" :placeholder="$t('custom_tabs.select_store_or_market')" :loading="loadingModels"
            class="w-full" :class="{ 'p-invalid': showAddValidationErrors && !newDetail.model_id }"
        ...
        <div class="field-group" :class="{ 'lg:col-span-2': newDetail.type !== 1 }">
          ...
          <MultiSelect v-model="newDetail.ids" :options="availableItems" :optionLabel="multiSelectLabel"
            optionValue="id" :placeholder="currentSelectPlaceholder(newDetail.type)"
            :filterPlaceholder="$t('custom_tabs.search') + ' ' + currentSelectLabel(newDetail.type).toLowerCase()"
            :loading="loadingItems" filter @filter="(e) => onItemFilter('new', e)" class="w-full"
        ...
        <div class="field-group lg:col-span-3">
          <label class="input-label"> {{ $t('custom_tabs.image') }} <span class="text-red-500">*</span> </label>
          <div class="flex items-center space-x-4">
            <FileUpload mode="basic" accept="image/*" :maxFileSize="2000000" @select="onImageSelect('new', $event)"
              @remove="newDetail.image = null" :chooseLabel="$t('custom_tabs.select_image')"
      ...
      <Button :label="$t('custom_tabs.add_detail')" icon="pi pi-plus" class="mt-8 px-6 py-3 button-primary"
        @click="addDetail" :loading="addingDetail" />
    </div>
    ...
    <div class="card-fancy mb-8">
      ...
      <div v-if="tabDetails?.details?.length"
        ...
        <div v-for="detail in tabDetails.details" :key="detail.id" class="detail-card">
          <div v-if="detail.media?.length" class="h-40 overflow-hidden">
            <img :src="detail.media[0].url" :alt="detail.name_en"
          ...
          <div class="p-5">
            <div class="flex justify-between items-start mb-4">
              <div class="flex-1">
                ...
                <p class="text-sm text-gray-600 mb-2" :dir="$t('dir') === 'rtl' ? 'rtl' : 'ltr'">
                  {{ detail.name_ar }}
                </p>
            ...
            <div class="flex gap-3 justify-end pt-4 border-t border-gray-100">
              <Button icon="pi pi-pencil" class="p-button-rounded p-button-sm p-button-info"
                @click="openEditDialog(detail)" />
              <Button icon="pi pi-trash" severity="danger" class="p-button-rounded p-button-sm p-button-danger"
                @click="confirmDeleteDetail(detail.id)" />
            </div>
          </div>
        </div>
      </div>
      ...
      <p v-else
        class="text-center text-gray-500 py-12 text-lg italic border-2 border-dashed border-gray-300 rounded-xl m-4">
    ...
    <!-- Edit Dialog -->
    <Dialog v-model:visible="showEditDialog" :header="$t('custom_tabs.edit_detail')" :modal="true"
      class="w-full max-w-5xl" :class="{ rtl: $t('dir') === 'rtl' }" @hide="resetEditForm">
      <div v-if="editingDetail" class="p-fluid">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div class="field-group">
            <label class="input-label">{{ $t('custom_tabs.name_en') }} <span class="text-red-500">*</span></label>
            <InputText v-model="editingDetail.name_en" class="w-full"
              :class="{ 'p-invalid': showEditValidationErrors && !editingDetail.name_en }" />
          </div>

          <div class="field-group">
            <label class="input-label">{{ $t('custom_tabs.name_ar') }} <span class="text-red-500">*</span></label>
            <InputText v-model="editingDetail.name_ar" class="w-full"
              :class="{ 'p-invalid': showEditValidationErrors && !editingDetail.name_ar }" />
          </div>

          <div class="field-group">
            <label class="input-label">{{ $t('custom_tabs.type') }}</label>
            <Dropdown v-model="editingDetail.type" :options="detailTypeOptions" optionLabel="label" optionValue="value"
              class="w-full bg-gray-100 cursor-not-allowed" disabled />
          </div>

          <div v-if="editingDetail.type === 1" class="field-group">
            <label class="input-label">{{ $t('custom_tabs.select_store_market') }} <span
                class="text-red-500">*</span></label>
            <Dropdown v-model="editingDetail.model_id" :options="editModelOptions" :optionLabel="modelOptionLabel"
              optionValue="id" :placeholder="$t('custom_tabs.select_store_or_market')" :loading="editLoadingModels"
              class="w-full" @change="fetchCategoriesForModel('edit')" />
          </div>

          <div class="field-group" :class="{ 'lg:col-span-2': editingDetail.type !== 1 }">
            <label class="input-label">{{ currentSelectLabel(editingDetail.type) }} <span
                class="text-red-500">*</span></label>
            <MultiSelect v-model="editingDetail.ids" :options="editAvailableItems" :optionLabel="multiSelectLabel"
              optionValue="id" :placeholder="currentSelectPlaceholder(editingDetail.type)" :loading="editLoadingItems"
              filter @filter="(e) => onItemFilter('edit', e)" class="w-full"
          ...
          <div class="field-group lg:col-span-3">
            ...
            <div class="flex items-center space-x-4">
              <FileUpload mode="basic" accept="image/*" :maxFileSize="2000000" @select="onImageSelect('edit', $event)"
                @remove="editingDetail.newImage = null" :chooseLabel="editingDetail.newImage ? $t('custom_tabs.change_image') : $t('custom_tabs.select_new_image')
      ...
      <template #footer>
        <Button :label="$t('custom_tabs.cancel')" icon="pi pi-times" class="p-button-text"
          @click="showEditDialog = false" />
        <Button :label="$t('custom_tabs.save')" icon="pi pi-check" class="button-primary" @click="updateDetail"
          :loading="updatingDetail" />
      </template>
    </Dialog>
  </div>
</template>
  ...
  try {
    const { data } = await axios.get(endpoints[type], {
      params: {
        ...
        limit: (tabDetails.value.details.find(d => d.id === editingDetail.value.id).ids).length,
        whereInIds: JSON.stringify(tabDetails.value.details.find(d => d.id === editingDetail.value.id).ids).slice(1, -1),
      },
    })
...
const debouncedFetchItemsNew = debounce(
  () => fetchItemsByType(newDetail.value.type, itemSearchQuery.value.new, 'new'),
  400,
)
const debouncedFetchItemsEdit = debounce(
  () => fetchItemsByType(editingDetail.value?.type, itemSearchQuery.value.edit, 'edit'),
  400,
)
...
const deleteDetail = async (id) => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: t('error'),
      detail: err.response?.data?.message || t('custom_tabs.delete_failed'),
      life: 5000,
    })
  }
}
